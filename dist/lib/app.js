            ...
            return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                try {
                    ...
                    db.fractal().then(result => {
                        console.log(chalk_1.default.green.inverse(result));
                    }).then(error => {
                        console.log(chalk_1.default.red.inverse(error));
                    });
                }
                catch (error) {
                    throw error;
        ...
        this.run = (port) => __awaiter(this, void 0, void 0, function* () {
            try {
                this.app.listen(port, () => {
                    console.log(chalk_1.default.green.inverse(`${Config.Server.name} listening on the port ${port}`));
                    ...
                    if (err.code == 'EADDRINUSE')
                        console.log(chalk_1.default.red.inverse(`${Config.Server.name} failed listening on the port ${err['port']}`));
                    console.log(chalk_1.default.red.inverse(`${Config.Server.name} try listening on the port ${next}`));
                    yield this.run(next);
                }));
            }
            catch (error) {
